...
message("=== Starting Gene Set Enrichment Analysis (GSEA) for sensitivity cohort of GSE193677 ===")
...
res_go <- gsea_lollipop(go_df, db_name = "GO", color = "#1B7837", title = "GSEA enrichment analysis – Sensitivity Cohort - GO terms (BP/MF/CC)")
...
res_kegg <- gsea_lollipop(kegg, db_name = "clusterProfiler", color = "#762A83", title = "GSEA enrichment analysis – Sensitivity Cohort - KEGG pathways")

kegg_tree <- add_redundancy_treeplot(
  kegg, title = "KEGG GSEA (redundancy tree) - CD vs Control",
  out_dir = paths$plots_bulk_rnaseq_sensitivity_gsea, filename = "GSEA_kegg_treeplot.png"
)
...
res_reactome <- gsea_lollipop(reactome, db_name = "clusterProfiler", color = "darkorange3", title = "GSEA enrichment analysis – Sensitivity Cohort - Reactome pathways")

reactome_tree <- add_redundancy_treeplot(
  reactome, title = "Reactome GSEA (redundancy tree) - CD vs Control",
  out_dir = paths$plots_bulk_rnaseq_sensitivity_gsea, filename = "GSEA_reactome_treeplot.png"
)
...
res_h <- gsea_lollipop(obj = hallmarks_H, db_name = "Hallmark", color = "firebrick", title = "GSEA enrichment analysis – Sensitivity Cohort - Hallmark H pathways")
...
res_c7 <- gsea_lollipop(obj = hallmarks_C7, db_name = "Hallmark", color = "darkgreen", title = "GSEA enrichment analysis – Sensitivity Cohort - Immunologic Signatures C7 (ImmunoSigDB) pathways")
...
save_csv(go_df,               filename = "results_GSE193677_sensitivity_GSEA_go_df.csv",         dir = paths$tables_bulkrnaseq)
save_csv(kegg_df,             filename = "results_GSE193677_sensitivity_GSEA_kegg_df.csv",       dir = paths$tables_bulkrnaseq)
save_csv(reactome_df,         filename = "results_GSE193677_sensitivity_GSEA_reactome_df.csv",   dir = paths$tables_bulkrnaseq)
save_csv(hallmarks_H_df_clean,  filename = "results_GSE193677_sensitivity_GSEA_hallmarks_h_df.csv",  dir = paths$tables_bulkrnaseq)
save_csv(hallmarks_C7_df_clean, filename = "results_GSE193677_sensitivity_GSEA_ImmunoSigDB_c7_df.csv", dir = paths$tables_bulkrnaseq)
...
save_plot(res_go[[1]],       filename = "GSEA_go.png",           dir = paths$plots_bulk_rnaseq_sensitivity_gsea, width = 20, height = 9)
save_plot(res_kegg[[1]],     filename = "GSEA_kegg.png",         dir = paths$plots_bulk_rnaseq_sensitivity_gsea, width = 11, height = 9)
save_plot(res_reactome[[1]], filename = "GSEA_reactome.png",     dir = paths$plots_bulk_rnaseq_sensitivity_gsea, width = 11, height = 9)
save_plot(res_h[[1]],        filename = "GSEA_hallmarks_H.png",  dir = paths$plots_bulk_rnaseq_sensitivity_gsea, width = 11, height = 9)
save_plot(res_c7[[1]],       filename = "GSEA_ImmunoSigDB_C7.png", dir = paths$plots_bulk_rnaseq_sensitivity_gsea, width = 15, height = 9)
...
save_session_info(filename = "sessionInfo_GSEA_Sensitivity_GSE193677.txt", dir = paths$logs, label = "Gene Set Enrichment Analysis (GSEA) - Sensitivity cohort of GSE193677")
