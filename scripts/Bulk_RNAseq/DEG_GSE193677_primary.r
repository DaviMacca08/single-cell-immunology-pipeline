...
# Script       : Quality Control, Exploratory Data Analysis and Differential Expression Analysis (Primary analysis)
...
message("=== Starting GSE193677 RNA-seq primary analysis ===")
...
# =========================================================
#                  Define primary analysis subset
# =========================================================

message("[SUBSET] Defining primary analysis cohort: ileal NonI CD and Control biopsies.")

sub_ileum_nonI <- sub_ileum[sub_ileum$infl == "NonI", ,drop = FALSE]

message("[SUBSET] Primary cohort contains ", nrow(sub_ileum_nonI), " biopsies.")

message("[CHECK] Primary cohort disease distribution:")

print(table(sub_ileum_nonI$disease))


# =========================================================
#                  Primary cohort replication check
# =========================================================

message("[CHECK] Verifying one ileal NonI biopsy per patient...")

primary_biopsies_per_patient <- table(sub_ileum_nonI$patient_id)

print(table(primary_biopsies_per_patient))

if (any(primary_biopsies_per_patient > 1)) {
  
  stop("[CHECK] More than one NonI ileal biopsy detected for at least one patient.")
  
}

message("[CHECK] Primary cohort contains one NonI ileal biopsy per patient.")
...
message("[SUMMARY] Primary analysis cohort:")

print(
  data.frame(
    Cohort = "Ileum_NonI_CD_vs_Control",
    CD = sum(sub_ileum_nonI$disease == "CD"),
    Control = sum(sub_ileum_nonI$disease == "Control"),
    Total = nrow(sub_ileum_nonI),
    Unique_Patients = length(
      unique(sub_ileum_nonI$patient_id)
    )
  )
)
...
# =========================================================
#                  Define primary count matrices
# =========================================================

message("[COUNTS] Creating primary-specific count matrices...")

stopifnot(all(sub_ileum_nonI$sample %in% colnames(counts_raw)))

counts_primary <- counts_raw[, sub_ileum_nonI$sample, drop = FALSE]


# =========================================================
#                  Primary cohort alignment
# =========================================================

message("[CHECK] Validating primary count matrix alignment...")

meta_primary <- sub_ileum_nonI[match(colnames(counts_primary), sub_ileum_nonI$sample), , drop = FALSE]

if (!identical(colnames(counts_primary), meta_primary$sample)) {
  stop(
    "[CHECK] Primary count matrix and metadata are not aligned."
  )
}

message(
  "[CHECK] Primary counts and metadata are correctly aligned."
)
...
message("[QC] Checking primary count matrix...")

stopifnot(ncol(counts_primary) == nrow(meta_primary))

stopifnot(nrow(counts_primary) > 1000)

stopifnot(!any(duplicated(rownames(counts_primary))))

stopifnot(!any(duplicated(colnames(counts_primary))))

stopifnot(!anyNA(as.matrix(counts_primary)))

stopifnot(all(is.finite(as.matrix(counts_primary))))

stopifnot(all(counts_primary >= 0))

# Verify sample order

stopifnot(all(colnames(counts_primary) == meta_primary$sample))

message(
  "[QC] Primary count matrix: ",
  nrow(counts_primary),
  " genes x ",
  ncol(counts_primary),
  " samples."
...
colData <- data.frame(
  row.names = meta_primary$sample,
  condition = factor(meta_primary$disease, levels = c("Control", "CD"))
)
...
stopifnot(all(colnames(counts_primary) == rownames(colData)))
...
dge <- edgeR::DGEList(counts = counts_primary)
...
open_png(filename = "voom_mean_variance_trend.png", dir = paths$plots_bulk_rnaseq_primary_qc,
...
weights_plot <- ggplot(sample_weights_df, aes(x = sample_weight, fill = group)) +
  ...
  labs(title = "Distribution of empirical sample quality weights (GSE193677, Primary)",
       x = "Sample weight (voomLmFit)", y = "Count") +
  theme_bw()

save_plot(weights_plot, filename = "sample_quality_weights_distribution.png",
          dir = paths$plots_bulk_rnaseq_primary_qc, width = 8, height = 6)

# Distribution of voom log2-CPM values

open_png(filename = "boxplot_logCPM_distributions.png", dir = paths$plots_bulk_rnaseq_primary_qc,
         width = 1600, height = 1300)

par(mar = c(8, 4, 4, 2))
boxplot(logcpm,
        main = "Distribution of voom log2-CPM per sample (GSE193677, Primary)",
        ylab = "log2-CPM",
        col = sample_colors,
...
open_png(filename = "density_plot_logCPM.png", dir = paths$plots_bulk_rnaseq_primary_qc, 
...
libsize_plot <- ggplot(lib_df, aes(x = reorder(sample, lib_size), y = lib_size, fill = group)) +
  ...
  coord_flip() +
  scale_fill_manual(values = group_palette) +
  labs(title = "Effective library size per sample (GSE193677, Primary)", x = "", y = "TMM-normalized library size") +
  theme_bw() + 
  theme(axis.title.y = element_text(size = 0.5))

save_plot(libsize_plot, filename = "library_size.png", dir = paths$plots_bulk_rnaseq_primary_qc, width = 11, height = 15)
...
open_png(filename = "outlier_median_logCPM.png", dir = paths$plots_bulk_rnaseq_primary_qc,
...
corr_check_plot <- ggplot(corr_diag_df, aes(x = group, y = avg_within_group_cor, color = group)) +
  ...
  labs(title = "Average within-group sample correlation (GSE193677, Primary)",
       x = "", y = "Average within-group Pearson correlation") +
  theme_bw()

save_plot(corr_check_plot, filename = "outlier_within_group_correlation.png",
          dir = paths$plots_bulk_rnaseq_primary_qc, width = 7, height = 6)
...
pca_plot_group <- ggplot(pca_df, aes(PC1, PC2, color = group, shape = outlier)) +
  ...
  ggtitle("PCA of GSE193677 samples (Primary: Ileum NonI, CD vs Control)") +
  theme_bw()

save_plot(pca_plot_group, filename = "pca_plot.png", dir = paths$plots_bulk_rnaseq_primary_eda)
...
open_png(filename = "pca_scree_plot.png", dir = paths$plots_bulk_rnaseq_primary_eda,
         width = 1000, height = 700)
...
open_png(filename = "hierarchical_clustering.png", dir = paths$plots_bulk_rnaseq_primary_eda, 
...
open_png(filename = "sample_correlation_heatmap.png", dir = paths$plots_bulk_rnaseq_primary_eda,
...
pheatmap::pheatmap(
  ...
  main = "Sample-sample Pearson correlation (GSE193677, Primary)"
)
...
open_png(filename = "sample_distance_heatmap.png", dir = paths$plots_bulk_rnaseq_primary_eda,
         width = 1400, height = 1400)
...
open_png(filename = "pvalue_histogram.png", dir = paths$plots_bulk_rnaseq_primary_deg,
  width = 1000, height = 700)
...
hist(
  ...
  sub = "CD vs Control | GSE193677 | Primary cohort",
...
volcano_plot_deg <- ggplot(deg_table, aes(x = logFC, y = -log10(adj.P.Val), color = significance)) +
  ...
  labs(
    title = "Volcano plot: CD vs Control (GSE193677, Ileum NonI, Primary Cohort)",
...
save_plot(volcano_plot_deg, filename = "volcano_plot.png", dir = paths$plots_bulk_rnaseq_primary_deg,
          width = 8, height = 6)

message("[DEG] Generating MA plot...")

open_png(filename = "MA_plot_CD_vs_Control.png", dir = paths$plots_bulk_rnaseq_primary_deg, 
         width = 1000, height = 800)
...
limma::plotMA(fit2, coef = "CD_vs_Control",
              ...
              main = "MA plot: CD vs Control (GSE193677, Primary Cohort)")
...
open_png(filename = "Heatmap_TopDegs_CD_vs_Control.png", dir = paths$plots_bulk_rnaseq_primary_deg,
...
ComplexHeatmap::Heatmap(
  ...
  column_title = sprintf("Top %d Differentially Expressed Genes (Primary Cohort: CD vs Control)", top_degs),
...
save_rds(fit, filename = "voomLmFit_object_primary.rds", dir = paths$objects_bulkrnaseq)
save_rds(fit2, filename = "limma_fit2_primary.rds", dir = paths$objects_bulkrnaseq)

message("[OUTPUT] Saving differential expression results...")

save_csv(deg_table, filename = "DEGs_GSE193677_primary.csv", dir = paths$tables_bulkrnaseq)
...
save_csv(background, filename = "background_GSE193677_primary.csv", dir = paths$tables_bulkrnaseq)

message("[OUTPUT] Saving outliers tables...")

save_csv(qc_outliers_df, filename = "qc_outliers_primary.csv", dir = paths$tables_bulkrnaseq)
...
save_session_info(filename = "sessionInfo_DEG_GSE193677_primary.txt", dir = paths$logs,
                  label = "Differential expression analysis - GSE193677 (Primary: Ileum NonI, CD vs Control)")
...
message("=================================================")
message("[PIPELINE] Differential expression analysis completed successfully for GSE193677 (Primary: Ileum NonI, CD vs Control).")
message("=================================================")
