...
message("=== Starting over-representation primary analysis for GSE193677 ===")
...
background <- read.csv(file = file.path(paths$tables_bulkrnaseq, "background_GSE193677_primary.csv"), sep = "," )
...
go_all_plot <- plot_enrichment(go_all_df, "GO Enrichment - Primary Cohort: CD vs Control (All genes)", color = "#C44E52")
go_up_plot <- plot_enrichment(go_up_df, "GO Enrichment - Primary Cohort: CD vs Control (Up-regulated genes)", color = "#009E73")
go_down_plot <- plot_enrichment(go_down_df, "GO Enrichment - Primary Cohort: CD vs Control (Down-regulated genes)", color = "#CC79A7")

# Save enrichment plots

save_plot(go_all_plot, filename = "GO_all_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
save_plot(go_up_plot, filename = "GO_Up_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
save_plot(go_down_plot, filename = "GO_Down_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
...
kegg_all_plot <- plot_enrichment(kegg_all, title = "KEGG Enrichment - Primary Cohort: CD vs Control (All genes)", color = "#C44E52")
kegg_up_plot <- plot_enrichment(kegg_up, title = "KEGG Enrichment - Primary Cohort: CD vs Control (Up-regulated genes)", color = "#009E73")
kegg_down_plot <- plot_enrichment(kegg_down, title = "KEGG Enrichment - Primary Cohort: CD vs Control (Down-regulated genes)", color = "#CC79A7")

# Generate treeplots

kegg_all_tree  <- add_redundancy_treeplot(kegg_all,  "KEGG (redundancy tree) - Primary Cohort: CD vs Control (All genes)",  paths$plots_bulk_rnaseq_primary_ora, "KEGG_all_genes_treeplot.png")
kegg_up_tree   <- add_redundancy_treeplot(kegg_up,   "KEGG (redundancy tree) - Primary Cohort: CD vs Control (Up-regulated genes)", paths$plots_bulk_rnaseq_primary_ora, "KEGG_up_genes_treeplot.png")
kegg_down_tree <- add_redundancy_treeplot(kegg_down, "KEGG (redundancy tree) - Primary Cohort: CD vs Control (Down-regulated genes)", paths$plots_bulk_rnaseq_primary_ora, "KEGG_down_genes_treeplot.png")

# Save enrichment plots

save_plot(kegg_all_plot, filename = "KEGG_all_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
save_plot(kegg_down_plot, filename = "KEGG_down_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
...
react_all_plot <- plot_enrichment(react_all, title = "Reactome Enrichment - Primary Cohort: CD vs Control (All genes)", color = "#C44E52")
react_up_plot <- plot_enrichment(react_up, title = "Reactome Enrichment - Primary Cohort: CD vs Control (Up-regulated genes)", color = "#009E73")
react_down_plot <- plot_enrichment(react_down, title = "Reactome Enrichment - Primary Cohort: CD vs Control (Down-regulated genes)", color = "#CC79A7")

# Generate treeplots

react_all_tree  <- add_redundancy_treeplot(react_all,  "Reactome (redundancy tree) - Primary Cohort: CD vs Control (All genes)",  paths$plots_bulk_rnaseq_primary_ora, "Reactome_all_genes_treeplot.png")
react_up_tree   <- add_redundancy_treeplot(react_up,   "Reactome (redundancy tree) - Primary Cohort: CD vs Control (Up-regulated genes)", paths$plots_bulk_rnaseq_primary_ora, "Reactome_up_genes_treeplot.png")
react_down_tree <- add_redundancy_treeplot(react_down, "Reactome (redundancy tree) - Primary Cohort: CD vs Control (Down-regulated genes)", paths$plots_bulk_rnaseq_primary_ora, "Reactome_down_genes_treeplot.png")

# Save enrichment plots

save_plot(react_all_plot, filename = "Reactome_all_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
save_plot(react_up_plot, filename = "Reactome_up_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
save_plot(react_down_plot, filename = "Reactome_down_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
...
hallmark_all_plot_h  <- plot_enrichment(hallmark_all_h,  title = "Hallmark H Enrichment - Primary Cohort: CD vs Control (All genes)", color = "#C44E52")
hallmark_up_plot_h   <- plot_enrichment(hallmark_up_h,   title = "Hallmark H Enrichment - Primary Cohort: CD vs Control (Up-regulated genes)", color = "#009E73")
hallmark_down_plot_h <- plot_enrichment(hallmark_down_h, title = "Hallmark H Enrichment - Primary Cohort: CD vs Control (Down-regulated genes)", color = "#CC79A7")

# Save enrichment plots

save_plot(hallmark_up_plot_h,   filename = "Hallmark_H_up_genes.png",   dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
...
hallmark_all_plot_c7  <- plot_enrichment(hallmark_all_c7,  title = "Immunologic Signatures C7 Enrichment - Primary Cohort: CD vs Control (All genes)", color = "#C44E52")
hallmark_up_plot_c7   <- plot_enrichment(hallmark_up_c7,   title = "Immunologic Signatures C7 Enrichment - Primary Cohort: CD vs Control (Up-regulated genes)", color = "#009E73")
hallmark_down_plot_c7 <- plot_enrichment(hallmark_down_c7, title = "Immunologic Signatures C7 Enrichment - Primary Cohort: CD vs Control (Down-regulated genes)", color = "#CC79A7")

# Generate treeplots

hallmark_all_c7_tree  <- add_redundancy_treeplot(hallmark_all_c7,  title = "Immunologic Signatures C7 (redundancy tree) - Primary Cohort: CD vs Control (All genes)",  paths$plots_bulk_rnaseq_primary_ora, "ImmunoSigDB_C7_all_genes_treeplot.png")
hallmark_up_c7_tree   <- add_redundancy_treeplot(hallmark_up_c7,   title = "Immunologic Signatures C7 (redundancy tree) - Primary Cohort: CD vs Control (Up-regulated genes)", paths$plots_bulk_rnaseq_primary_ora, "ImmunoSigDB_C7_up_genes_treeplot.png")
hallmark_down_c7_tree <- add_redundancy_treeplot(hallmark_down_c7, title = "Immunologic Signatures C7 (redundancy tree) - Primary Cohort: CD vs Control (Down-regulated genes)", paths$plots_bulk_rnaseq_primary_ora, "ImmunoSigDB_C7_down_genes_treeplot.png")

# Save enrichment plots

save_plot(hallmark_all_plot_c7,  filename = "ImmunoSigDB_C7_all_genes.png",  dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
save_plot(hallmark_down_plot_c7, filename = "ImmunoSigDB_C7_down_genes.png", dir = paths$plots_bulk_rnaseq_primary_ora, width = 10, height = 9)
...
save_session_info(filename = "sessionInfo_ORA_primary_GSE193677.txt", dir = paths$logs, label = "Over-Representation Analysis (ORA) - Primary cohort GSE193677")
...
message("=================================================")
message("[PIPELINE] Over-Representation Analysis completed successfully for primary analysis GSE193677 (CD vs  Control).")
message("=================================================")
