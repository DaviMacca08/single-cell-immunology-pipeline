...
# Compartment  : LPL (Lamina Propria Lymphocytes)
...
message("=== Starting LPL compartment GSEA ===")
...
message("[LOAD] Reading pseudobulk DEG results (LPL)...")

deg_list <- readRDS(file.path(paths$objects_pseudobulk, "results_list_LPL.rds"))
...
for (ct in celltype_DE) {
  ...
  if (!is.null(res_reactome)) {
    save_plot(res_reactome$plot, filename = paste0("GSEA_Reactome_", ct_tag, ".png"),
              dir = paths$plots_gsea_LPL, width = 11, height = 9)
  }
  ...
  if (!is.null(res_go)) {
    save_plot(res_go$plot, filename = paste0("GSEA_GOBP_", ct_tag, ".png"),
              dir = paths$plots_gsea_LPL, width = 12, height = 9)
  }
  ...
  hallmarks_C7_df_clean <- as.data.frame(hallmarks_C7)[, !(names(hallmarks_C7) %in% "leadingEdge")]
  hallmarks_C7 <- clean_hallmark_names(hallmarks_C7, prefix = "")
  
  c7_list[[ct]] <- hallmarks_C7
  
  res_c7 <- gsea_lollipop(
    hallmarks_C7, db_name = "C7", color = "darkgreen",
  ...
  if (!is.null(res_c7)) {
    save_plot(res_c7$plot, filename = paste0("GSEA_C7_", ct_tag, ".png"),
              dir = paths$plots_gsea_LPL, width = 15, height = 9)
  }
  ...
  save_csv(hallmarks_H_df_clean, filename = paste0("GSEA_hallmark_H_", ct_tag, "_LPL.csv"), dir = paths$tables_gsea_LPL)
  save_csv(hallmarks_C7_df_clean, filename = paste0("GSEA_C7_", ct_tag, "_LPL.csv"), dir = paths$tables_gsea_LPL)
  save_csv(as.data.frame(reactome), filename = paste0("GSEA_reactome_", ct_tag, "_LPL.csv"), dir = paths$tables_gsea_LPL)
  save_csv(as.data.frame(go_bp_simplified), filename = paste0("GSEA_GOBP_", ct_tag, "_LPL.csv"), dir = paths$tables_gsea_LPL)
...
message("[OUTPUT] Saving GSEA result objects (all cell types, LPL)...")

save_rds(ranked_list,   filename = "GSEA_ranked_lists_LPL.rds",   dir = paths$objects_pseudobulk)
save_rds(reactome_list, filename = "GSEA_reactome_list_LPL.rds",  dir = paths$objects_pseudobulk)
save_rds(go_list,       filename = "GSEA_GOBP_list_LPL.rds",      dir = paths$objects_pseudobulk)
save_rds(hallmark_list, filename = "GSEA_hallmark_list_LPL.rds",  dir = paths$objects_pseudobulk)
save_rds(c7_list,       filename = "GSEA_C7_list_LPL.rds",        dir = paths$objects_pseudobulk)
...
save_session_info(filename = "sessionInfo_GSEA_LPL.txt", dir = paths$logs, label = "Gene Set Enrichment Analysis (GSEA) - LPL compartment")
...
message("=================================================")
message("[PIPELINE] Gene Set Enrichment Analysis completed successfully for LPL compartment (CD vs Control, per cell type).")
message("=================================================")
