...
cell_count <- tibble(sample = names(sample_list), n_cells = sapply(sample_list, ncol))
...
save_rds(cd_harmony, filename = "srt_obj_harmony.rds", dir = paths$objects)
save_rds(seurat_merge, filename = "srt_obj_pca.rds", dir = paths$objects)

message("Integration pipeline completed successfully")
cd_harmony <- readRDS("objects/srt_obj_harmony.rds")
