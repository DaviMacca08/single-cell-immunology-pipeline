...
Compartment-specific reclustering (IEL, LPL) was performed on RNA  assay layers with a hybrid join state inherited from the global object: the `data` layer was joined across samples, while `counts` layers remained split by sample. As a consequence, variable feature selection followed a per-sample ranking procedure (Seurat v5 default behavior for split layers) rather than a fully pooled computation. 
This provides partial, unintentional mitigation of inter-sample technical variation, distinct from and weaker than formal batch integration. Downstream functional enrichment results (see REPORT_GSEA) showing biologically coherent, literature-consistent signatures (e.g., TH17 inflammatory activation, cytotoxic OXPHOS programs matching Jaeger et al. 2021) provide indirect support that cluster structure is not dominated by residual technical variation, though a formal sensitivity comparison (e.g., against a fully-joined or Harmony-integrated reanalysis) was not performed.
...
-   consensus annotation established at the global level prior to 
    compartment-specific reclustering
-   retention of the full-resolution global object for cross-reference
