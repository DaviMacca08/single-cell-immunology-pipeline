...
cd_harmony <- readRDS(
  file.path(paths$objects_global, "srt_obj_annotated_SingleR_Azimuth.rds")
)
...
save_csv(cluster_annotations, filename = "cluster_consensus_annotation_global.csv", dir = paths$tables_markers)

message("Saving annotated Seurat object...")

save_rds(cd_harmony, filename = "srt_consensus_celltype.rds", dir = paths$objects_global)
...
save_session_info(filename = "sessionInfo_Consensus_Global_CellType.txt", dir = paths$logs, label = "Consensus celltype annotations")
