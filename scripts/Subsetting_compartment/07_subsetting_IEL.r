...
message("Checking RNA assay structure...")

DefaultAssay(cd_harmony) <- "RNA"

print(Layers(cd_harmony[["RNA"]]))

message(
  "Total cells in Seurat object: ",
  ncol(cd_harmony)
)

message(
  "Cells in RNA data layer: ",
  ncol(
    LayerData(
      cd_harmony,
      assay = "RNA",
...
message("Checking sample representation in RNA data layer...")
...
print(table(cd_harmony$sample_id))

print(table(cd_harmony$sample_id[data_cells]))
...
# =========================================================
#             IEL subset validation
# =========================================================

message("Checking IEL subset integrity...")
...
message(
  "IEL samples represented:"
)

print(table(iel_cells$sample_id))
...
# ---------------------------------------------------------
# Check consistency between metadata and expression matrix
# ---------------------------------------------------------

message("Checking metadata rownames match Seurat object colnames...")
stopifnot(
  identical(rownames(iel_cells@meta.data), colnames(iel_cells))
)
message("Metadata rownames and object colnames are identical.")

# ---------------------------------------------------------
# If layers are split by sample (Seurat v5), check total sums
# ---------------------------------------------------------

message("Checking for split layers (Seurat v5 style)...")
layer_names <- Layers(iel_cells, assay = "RNA", search = "data")
message("Data layers found: ", paste(layer_names, collapse = ", "))

if (length(layer_names) > 1) {
  total_split <- sum(
    sapply(
      layer_names,
      function(l) ncol(LayerData(iel_cells, assay = "RNA", layer = l))
    )
  )
  message(
    "Sum of columns across split layers: ", total_split,
    " vs total ncol(iel_cells): ", ncol(iel_cells)
  )
  stopifnot(total_split == ncol(iel_cells))
  message("Split layers sum matches total cell count.")
} else {
  message("No split layers detected (single 'data' layer present).")
}

message("IEL subset integrity check completed successfully.")
