# =========================================================
# Pseudobulk differential expression analysis - IEL
# =========================================================
# Input:
...
message("Starting pseudobulk analysis for IEL compartment...")
...
stopifnot(inherits(iel_cells, "Seurat"))
stopifnot("seurat_clusters" %in% colnames(iel_cells@meta.data))
...
celltype_plot_iel <- DimPlot(iel_cells, reduction = "umap", group.by = "celltype_IEL", label = TRUE, repel = TRUE) + 
  xlab("UMAP 1") + 
  ylab("UMAP 2")
...
iel_cells$celltype_pb <- dplyr::case_when(
  
  iel_cells$seurat_clusters %in% c(0, 1, 2, 10, 14, 15) ~ "IEL-Cytotoxic-TRM-like" , 
  
  iel_cells$seurat_clusters %in% c(4, 5, 11) ~ "IEL-TH17",

  iel_cells$seurat_clusters == 7  ~ "IEL-TFH",
  
  iel_cells$seurat_clusters == 17 ~ "IEL-Treg",
  
  iel_cells$seurat_clusters %in% c(3,6,8,9,12,13) ~ "IEL-Other-T-cells",

  iel_cells$seurat_clusters == 16 ~ "IEL-Cycling-T-cells",

  iel_cells$seurat_clusters == 18 ~ "Myeloid-contamination",

  iel_cells$seurat_clusters == 19 ~ "Epithelial-contamination",
  
  TRUE ~ NA_character_  
  
  )

stopifnot(!any(is.na(iel_cells$celltype_pb)))

# ---------------------
# DimPlot Major Groups
# ---------------------

celltype_plot_pb <- DimPlot(iel_cells, reduction = "umap", group.by = "celltype_pb", label = TRUE, repel = TRUE) +
  xlab("UMAP 1") + 
  ylab("UMAP 2") 

save_plot(celltype_plot_iel + celltype_plot_pb, filename = "DimPlot_IEL_MajorGroup.png", dir = paths$plots_pb_IEL,
          width = 20, height = 10)
...
tab_CellxDisease <- (prop.table(table(iel_cells$celltype_pb, iel_cells$disease), margin = 2))

com_heatmap_table(mat = tab_CellxDisease, filename = "HeatMap_IEL_CellxDisease.pdf", 
            title = "Cell type composition per disease (proportions) - IEL Compartment", dir = paths$plots_pb_IEL)

# ------------------------------
# Table celltype_pb x condition
# ------------------------------

tab_CellxCondition <- (prop.table(table(iel_cells$celltype_pb, iel_cells$condition), margin = 2))

com_heatmap_table(mat = tab_CellxCondition, filename = "HeatMap_IEL_CellxCondition.pdf", 
            title = "Cell type composition per condition (proportions) - IEL Compartment", dir = paths$plots_pb_IEL)

# ------------------------------
# Table celltype_pb x donor
# ------------------------------

tab_CellxDonor <- (prop.table(table(iel_cells$celltype_pb, iel_cells$donor), margin = 2))
 
com_heatmap_table(mat = tab_CellxDonor, filename = "HeatMap_IEL_CellxDonor.pdf", 
            title = "Cell type composition per donor (proportions) - IEL Compartment", dir = paths$plots_pb_IEL)

# ------------------------------
# Table condition x donor
# ------------------------------

tab_ConditionxDonor <- (prop.table(table(iel_cells$condition, iel_cells$donor), margin = 2))

com_heatmap_table(mat = tab_ConditionxDonor, filename = "HeatMap_IEL_ConditionxDonor.pdf", 
            title = "Cell type condition per donor (proportions) - IEL Compartment", dir = paths$plots_pb_IEL)

# ------------------------------
# Table condition x disease
# ------------------------------

tab_ConditionxDisease <- (prop.table(table(iel_cells$condition, iel_cells$disease), margin = 2))

com_heatmap_table(mat = tab_ConditionxDisease, filename = "HeatMap_IEL_ConditionxDisease.pdf", 
            title = "Cell type condition per disease (proportions) - IEL Compartment", dir = paths$plots_pb_IEL)
...
cell_counts <- as.data.frame(
  table(
    celltype_pb = iel_cells$celltype_pb,
    donor       = iel_cells$donor,
...
eligible_celltypes <- donor_summary |> 
  ...
  dplyr::filter(
    n_groups == dplyr::n_distinct(iel_cells$disease),
...
iel_cells_pb$pseudobulk_id <- paste(
  iel_cells_pb$celltype_pb,
  iel_cells_pb$donor,
  iel_cells_pb$disease,
  sep = "-"
)

message("Aggregate sample...")

DefaultAssay(iel_cells_pb) <- "RNA"
...
for (ct in celltype_DE) {
  ...
  open_png(
    ...
    dir = paths$plots_pb_IEL, width = 1000, height = 800
  )
  ...
  open_png(
    ...
    dir = paths$plots_pb_IEL, width = 1400, height = 1600
  )
...
save_rds(dds_list, filename = "results_list_IEL.rds", dir = paths$objects_pseudobulk)

save_rds(deg_list, filename = "deg_list_shrunk_IEL.rds", dir = paths$objects_pseudobulk)

save_csv(cts_db, filename = "aggregated_counts_IEL.csv", dir = paths$objects_pseudobulk)

message("Pseudobulk outputs for IEL compartment saved in: ", paths$objects_pseudobulk)
...
message("Saving session information (pseudobulk DESeq2 analysis for IEL compartment)...")

save_session_info(filename = "sessionInfo_pseudobulk_DESeq2_IEL.txt", dir = paths$logs, label = "Pseudobulk DESeq2 analysis for IEL compartment")
...
message("=================================================")
message("PIPELINE STEP COMPLETED: Pseudobulk differential expression for IEL compartment")
message("=================================================")