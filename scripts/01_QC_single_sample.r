...
save_plot(vln_p, "01-QC_ViolinPlot.png", dir = sample_plot_dir)
...
save_plot(scat1 + scat2, "02-QC_ScatterPlots.png",dir = sample_plot_dir)
...
save_plot(ridge1 + ridge2 + ridge3, "03-QC_DensityPlot.png",dir = sample_plot_dir)
...
srt_obj <- subset(srt_obj, subset = nFeature_RNA > 200 & nFeature_RNA < 3600 & nCount_RNA < 15000 & percent.mt < 13)
...
save_plot(vln_filtered, "04-QC_ViolinPlot_filtered.png", dir = sample_plot_dir)
...
message(paste("QC + preprocessing completed for sample:", unique(srt_obj$sample_id)))
...
writeLines(capture.output(sessionInfo()), file.path(paths$logs, paste0(unique(srt_obj$sample_id), "_sessionInfo.txt")))
