...
# =========================================================
#             LPL subset validation
# =========================================================

message("Checking LPL subset integrity...")
...
message(
  "LPL samples represented:"
)

print(table(lpl_cells$sample_id))
...
stopifnot(
  identical(rownames(lpl_cells@meta.data), colnames(lpl_cells))
)
...
layer_names <- Layers(lpl_cells, assay = "RNA", search = "data")
...
if (length(layer_names) > 1) {
  total_split <- sum(
    sapply(
      layer_names,
      function(l) ncol(LayerData(lpl_cells, assay = "RNA", layer = l))
    )
  )
  message(
    "Sum of columns across split layers: ", total_split,
    " vs total ncol(lpl_cells): ", ncol(lpl_cells)
  )
  stopifnot(total_split == ncol(lpl_cells))
  message("Split layers sum matches total cell count.")
} else {
  message("No split layers detected (single 'data' layer present).")
}

message("LPL subset integrity check completed successfully.")
